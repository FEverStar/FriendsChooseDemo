...
#import "YXUserSearchBar.h"
#import "YXUser.h"

#import "UITextField+YX.h"

@interface __YXUserSearchBarItem : UIButton
@property (strong, nonatomic) id<YXUser> user;
@end


@implementation __YXUserSearchBarItem

- (void)setUser:(id<YXUser>)user {
    _user = user;
...
@interface YXUserSearchBar ()<YXTextFieldDelegate>
@property (weak, nonatomic) NSLayoutConstraint *YX_scrollWidthConstraint;
@property (strong, nonatomic) NSMutableArray *dataList;
@property (strong, nonatomic) NSMutableArray<__YXUserSearchBarItem *> *items;

@property (strong, nonatomic) __YXUserSearchBarItem *searchItem;
@end


@implementation YXUserSearchBar
...
- (void)awakeFromNib {
    ...
    __YXUserSearchBarItem *search = [[__YXUserSearchBarItem alloc]initWithFrame:CGRectMake(0, 0, 44, 44)];
...
//    scrollView.automaticallyAdjustsScrollViewInsets = NO;
    ...
    self.YX_scrollWidthConstraint = ws_c;
...
//    __weak typeof(self) weakSelf = self;
    CGFloat margin = 8;
//    [self.dataList enumerateObjectsUsingBlock:^(id<YXUser> obj, NSUInteger idx, BOOL *stop) {
//        __YXUserSearchBarItem *item = [weakSelf.items objectForKey:obj.objID];
//        [item setFrame:CGRectMake(margin + idx * (40) , margin, 30, 30)];
//    }];
    
    
    [self.items enumerateObjectsUsingBlock:^(__YXUserSearchBarItem * _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
        [item setFrame:CGRectMake(margin + idx * (40) , margin, 30, 30)];
    }];
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField {
    if(!self.dataList.count) {
        self.YX_scrollWidthConstraint.constant = 10;
    }
...
- (BOOL)textFieldShouldEndEditing:(UITextField *)textField {
    if (!textField.text.length && !self.dataList.count) {
        if (![self.userScrollView.subviews containsObject:self.searchItem]) {
            self.YX_scrollWidthConstraint.constant = 50;
...
- (void)textFieldDidDeleteBackward:(UITextField *)textField {
    if (textField.text.length == 0) {
        id <YXUser>user = [self.dataList lastObject];
...
- (void)addScrollItem:(id <YXUser>)user {
    if ([self.dataList containsObject:user]) {
        __YXUserSearchBarItem *item = [[__YXUserSearchBarItem alloc]init];
        [item setUser:user];
        [item addTarget:self action:@selector(clickItem:) forControlEvents:UIControlEventTouchUpInside];
        [self.userScrollView addSubview:item];
//        [self.items YX_setObject:item forKey:user.objID];
...
- (void)clickItem:(__YXUserSearchBarItem *)item {
    [self removeUser:item.user];
}

- (BOOL)isSelectedItem:(id <YXUser>)user {
    if (!user) {
        return NO;
    }
    NSInteger index = [self.dataList indexOfObject:user];
    __YXUserSearchBarItem *item = [self.items objectAtIndex:index];
    if (item) {
...
- (void)unSelectedItems {
    ...
    [self.dataList enumerateObjectsUsingBlock:^(id <YXUser>obj, NSUInteger idx, BOOL *stop) {
        ...
        __YXUserSearchBarItem *item = [weakSelf.items objectAtIndex:index];
        if (item) {
...
- (void)selectItem:(id <YXUser>)user {
    if (!user) {
        return;
    }
    NSInteger index = [self.dataList indexOfObject:user];
...
- (void)removeScrollItem:(id <YXUser>)user {
    if (!user) {
        return;
    }
    NSInteger index = [self.dataList indexOfObject:user];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if ([keyPath isEqualToString:@"dataList"]) {
        ...
        if(![self.searchTextView isFirstResponder] && self.dataList.count == 0){
            ...
            self.YX_scrollWidthConstraint.constant = w;
            [self.searchItem removeFromSuperview];
        }
        [self.userScrollView setContentSize:CGSizeMake(w, 0)];
        [self.userScrollView setContentOffset:CGPointMake(w - self.YX_scrollWidthConstraint.constant, 0) animated:YES];
    }
}
@end
